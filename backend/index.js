...
async function searchYTS(query, limit = 20) {
  try {
    const response = await fetch(
      `${YTS_API_BASE}/list_movies.json?query_term=${encodeURIComponent(query)}&limit=${limit}`,
    );
...
app.get("/api/torrents/search", async (req, res) => {
  ...
  try {
    ...
    results.sort((a, b) => {
      ...
      const qualityDiff =
        (qualityOrder[bQuality] || 0) - (qualityOrder[aQuality] || 0);
    ...
    res.status(500).json({
      ...
      message:
        "The search service is temporarily unavailable. Please try again in a few minutes.",
      details:
